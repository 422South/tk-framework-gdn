...
from .symmetries import decode_string, encode_string, get_character
...
def decode_engineIO_content(content):
    logging.log(logging.DEBUG, "Raw: %s" % content)
    content_index = 0
    ...
    while content_index < content_length:
        ...
        logging.log(logging.DEBUG,
                    "PacketType: %s , Data: %s" % (_packet_type_to_str(engineIO_packet_type), engineIO_packet_data))
...
def format_packet_text(packet_type, packet_data):
    logging.log(logging.DEBUG, "Format Packet: %s,%s" % (_packet_type_to_str(packet_type), packet_data))
    return encode_string("%s%s" % (str(packet_type), packet_data))
...
def _packet_type_to_str(packet_type):
    packet_types = ['Open', 'Close', 'Ping', 'Pong', 'Message', 'Upgrade', 'NOOP']
