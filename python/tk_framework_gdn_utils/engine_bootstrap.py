...
def bootstrap(root_path, port, engine_name, app_id):
    ...
    if os.environ.get("TANK_CONTEXT") and os.environ.get("TANK_ENGINE"):
        logging.log(logging.WARNING, "Classic Bootstrap")
        tk_framework_gdn.toolkit_classic_bootstrap()

    else:
        logging.log(logging.WARNING, "Plugin Bootstrap")
        tk_framework_gdn.toolkit_plugin_bootstrap(root_path)
    ...
    app_name = "ShotGrid Framework for GDN"

    # create and set up the Qt app. we don't want the app to close when the
    # last window is shut down since it's running in parallel to the GDN product.
    ...
    engine.logger.debug("GDN Product: %s" % host_info["name"])
    engine.logger.debug("GDN Product: %s" % host_info["version"])
