...
sys.path.insert(
    ...
    os.path.normpath(
        os.path.join(
            os.path.dirname(__file__),  # ./python/tk_framework_gdn/rpc
            os.pardir,  # ./python/tk_framework_gdn
            os.pardir,  # ./python
            os.pardir,  # .
            "pkgs",  # ./pkgs.zip
        )
...
os.environ['SHOTGUN_ADOBE_NETWORK_DEBUG'] = "1"
...
class Communicator(object):
    ...
    def rpc_get(self, proxy_object, property_name):
        ...
        """
        self.log_network_debug("communicator Sending a get message using rpc_get...")
        self.log_network_debug(
            "communicator Getting property %s from object UID %s" % (property_name, proxy_object.uid)
        )
        ...
        try:
            ...
            self.log_network_debug("communicator rpc_get Got Runtime Error !!!")
    ...
    def _handle_response(self, response, *args):
        ...
        try:
            ...
            self.log_network_debug("Got to Type Error, Value Error")
            result = result.get("result")
            if result is six.text_type():
                result = six.ensure_str(result)
            self._RESULTS[uid] = result
        except KeyError:
            if True or not self._response_logging_silenced:
    ...
    def __run_rpc_command(
        ...
        """
        # self.log_network_debug("communicator __run_rpc_command %s" % six.ensure_str(params))
        ...
        self._io.emit(self._RPC_EXECUTE_COMMAND, payload)
        # self.log_network_debug("_RPC_EXECUTE_COMMAND %s" % six.ensure_str(payload))
        results = self._wait_for_response(payload["id"])
        # self.log_network_debug("Pre ERRORS")
        ...
        if isinstance(results, RuntimeError):
            raise RuntimeError()
        # self.log_network_debug("Got the response !!!!!!!")
